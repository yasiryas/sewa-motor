    ...
    <div class="container py-5">
        ...
        <div class="card shadow-lg border-0 rounded-lg p-4 bg-light">
            <div class="text-center mb-4">
                <h3 class="font-weight-bold text-dark mb-1">Nota Pembayaran</h3>
            ...
            <div class="row">
                ...
                <div class="col-lg-4 text-center">
                    ...
                    <h5 class="font-weight-bold mt-2"><?= esc($booking['brand_name']); ?> <?= esc($booking['motor_name']); ?></h5>
                    <p class="text-muted mb-0"><i class="fas fa-map-marker-alt"></i> <?= esc($booking['number_plate']); ?></p>
                </div>

                <!-- Detail Nota -->
                <div class="col-lg-8">
                    <div class="bg-white rounded-lg shadow-sm p-4">
                        <h5 class="font-weight-bold text-dark mb-3">Detail Transaksi</h5>
                        <table class="table table-borderless mb-0">
                            <tr>
                                <td><strong>Tanggal Pemesanan</strong></td>
                                <td class="text-right"><?= date('d F Y H:i', strtotime($booking['created_at'])); ?></td>
                            </tr>
                            <tr>
                                <td><strong>Tanggal Sewa</strong></td>
                                <td class="text-right"><?= date('d F Y', strtotime($booking['rental_start_date'])); ?></td>
                            </tr>
                            <tr>
                                <td><strong>Tanggal Kembali</strong></td>
                                <td class="text-right"><?= date('d F Y', strtotime($booking['rental_end_date'])); ?></td>
                            </tr>
                            <tr>
                                <td><strong>Harga per Hari</strong></td>
                                <td class="text-right">Rp <?= number_format($booking['price_per_day'], 0, ',', '.'); ?></td>
                            </tr>
                            <tr class="border-top">
                    ...
                    <form action="<?= base_url('booking/update-from-detail-user') ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-4 mt-4">
                            <h5 class="font-weight-bold text-dark mb-3">Pilih Metode Pembayaran</h5>
                            <?php
                            $selectedMethod = old('payment_method', $booking['payment_method']);
                            ?>
                            <div class="d-flex flex-column flex-md-row">
                                <button type="button" id="btnTransfer"
                                    class="method-btn w-100 p-3 rounded-lg border <?= $selectedMethod == 'transfer' ? 'active bg-primary text-white' : ''; ?>">
                                    <i class="fas fa-university fs-4 mb-2 d-block"></i>
                                    <strong>Transfer Bank</strong><br>
                                    <small>BCA / Mandiri / BRI</small>
                                </button>
                                <button type="button" id="btnCOD"
                                    class="method-btn w-100 p-3 rounded-lg border <?= $selectedMethod == 'cash' ? 'active bg-primary text-white' : ''; ?>">
                                    <i class="fas fa-money-bill-wave fs-4 mb-2 d-block"></i>
                            ...
                            <div id="rekeningSection" class="mt-4 bg-light p-3 rounded-3 <?= $selectedMethod == 'transfer' ? '' : 'd-none'; ?>">
                                <p class="font-weight-bold mb-1">Nomor Rekening:</p>
                            ...
                            <div id="CODSection" class="mt-4 bg-light p-3 rounded-3 <?= $selectedMethod == 'cash' ? '' : 'd-none'; ?>">
                                <p class="font-weight-bold mb-1">Pembayaran dilakukan di tempat (COD)</p>
                        ...
                        <!-- Form Keterangan -->
                        <div class="bg-white rounded-lg shadow-sm p-4 mt-4">
                            <h5 class="font-weight-bold text-dark mb-3">Catatan Pembeli</h5>
                        ...
                        <div class="bg-white rounded-lg shadow-sm p-4 mt-4">
                            <h5 class="font-weight-bold text-dark mb-3">Unggah Kartu Identitas (KTP/SIM)</h5>
                        ...
                        <!-- Bukti Pembayaran -->
                        <div class="bg-white rounded-lg shadow-sm p-4 mt-4 <?= $booking['payment_method'] == 'cash' ? 'd-none' : ''; ?>" id="buktiPembayaran">
                            <h5 class="font-weight-bold text-dark mb-3">Bukti Pembayaran</h5>
